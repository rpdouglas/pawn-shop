# The Pawn Shop Manual

Welcome to the central documentation for The Pawn Shop — a multi-vertical retail platform for Cornwall Island, Akwesasne. This guide covers the operational workflows for our four specialized storefronts.

## Core Pillars

### 📦 Inventory & Lifecycle
Understand how items move from a staff **Draft** to a live **Active** listing, including our automated image processing and watermark pipeline.
- [Lifecycle Overview](/inventory/lifecycle)
- [Staff Intake Process](/inventory/intake)
- [Merchandising & Staff Picks](/admin/merchandising)

### 🚬 Tobacco & Fine Smoking
Premium domestic and reserve tobacco products, with dedicated sections for cigars, cigarettes, and accessories.
- [Getting Started](/getting-started)
- [Age Verification (19+)](/compliance/age-gates)

### 🤝 Pawn & Resale
Details on our one-handed mobile pawn enquiry form, the server-side serial blacklist check, and our internal staff inbox management.
- [Browsing the Collection](/pawn/browsing)
- [Customer Pawn Enquiries](/pawn/selling)
- [Staff Inbox Management](/admin/pawn-inbox)
- [FAQ Management](/admin/faq-management)

### 🛡️ Storefronts & Compliance
Our platform manages high-stakes retail. Learn about our router-level age verification for Cannabis and Fireworks, our custom prefix-search discovery engine, and our regional search landing pages.
- [Age Verification (18+/19+)](/compliance/age-gates)
- [Search & Discovery](/inventory/search)
- [Regional Search (Local SEO)](/admin/local-seo)

### 🗓️ Operations & Config
Day-to-day tools for managing the shop floor — reservations, store hours, and staff coordination.
- [Reservations Inbox](/admin/reservations)
- [Store Hours](/admin/store-hours)
- [Staff Scheduling](/admin/scheduling)

### 💎 Editorial & Brand
Maintaining the "Dapper and Debonair" identity through our Markdown CMS and strict visual standards.
- [Editorial CMS](/admin/editorial-cms)
- [Branding & Visual Identity](/admin/branding)
- [Admin Desktop Portal](/admin/portal)

### 📈 Customers & Retention
Tools for tracking customer lifetime value (LTV), managing reseller tiers, and automated follow-ups.
- [Customers & Profiles](/admin/customers)
- [Saved Search Alerts](/pawn/alerts-notifications)
- [Seasonal Campaigns](/admin/campaigns)

### 🌐 Global Reach
Learn how we synchronize our Pawn inventory with eBay, including one-click cross-posting and real-time sold-sync via webhooks.
- [eBay Integration](/ebay/cross-posting)

---

## Safety & Trust
Safety is baked into the schema. Read about our **Police Hold** kill-switch and our immutable **Audit Logs** for regulatory accountability.
- [Police Holds](/admin/police-holds)
- [Audit Logging](/admin/audit-logs)
