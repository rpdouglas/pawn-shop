...
exports.sendWeeklyDigest = exports.sendPickupReminders = exports.sendSeasonalReminders = void 0;
const scheduler_1 = require("firebase-functions/v2/scheduler");
const firestore_1 = require("firebase-admin/firestore");
const sms_1 = require("@pawn-shop/shared/lib/sms");
const email_1 = require("@pawn-shop/shared/lib/email");
...
exports.sendSeasonalReminders = (0, scheduler_1.onSchedule)({ schedule: '0 * * * *', secrets: [secrets_1.twilioAccountSid, secrets_1.twilioAuthToken] }, async () => {
    ...
    for (const campaign of eligible) {
        ...
        for (const user of phoneRecipients) {
            try {
                const sent = await (0, sms_1.dispatchSms)(String(user.data()['phoneNumber']), smsBody);
                if (sent)
                    successCount++;
            }
            catch (err) {
        ...
        await campaign.ref.update({ reminderSentAt: firestore_1.FieldValue.serverTimestamp() });
...
exports.sendPickupReminders = (0, scheduler_1.onSchedule)({ schedule: '0 * * * *', secrets: [secrets_1.twilioAccountSid, secrets_1.twilioAuthToken] }, async () => {
    const db = (0, firestore_1.getFirestore)();
    const now = new Date();
    ...
    for (const doc of resSnap.docs) {
        const data = doc.data();
        if (data['pickupReminderSentAt'] != null)
            continue;
        const pickupWindow = String(data['pickupWindow'] ?? '');
        if (!pickupWindowStartsInRange(pickupWindow, from, to))
            continue;
        ...
        if (uid) {
            ...
            if (!userSnap.exists || userSnap.data()?.['alertOptIn'] !== true)
                continue;
        }
        const body = `The Pawn Shop — reminder: your pickup is tomorrow. Window: ${pickupWindow}. We look forward to seeing you.`;
        try {
            const sent = await (0, sms_1.dispatchSms)(phone, body);
            if (sent) {
                await doc.ref.update({ pickupReminderSentAt: firestore_1.Timestamp.now() });
                await db.collection('auditLogs').add({
            ...
            console.error(`[sendPickupReminders] Reservation ${doc.id} SMS failed:`, err.message);
        }
    }
    // ── Preorders ─────────────────────────────────────────────────────────────
    ...
    for (const doc of preSnap.docs) {
        ...
        if (data['pickupReminderSentAt'] != null)
            continue;
        const pickupWindow = String(data['pickupWindow'] ?? '');
        if (!pickupWindow || !pickupWindowStartsInRange(pickupWindow, from, to))
            continue;
        const phone = String(data['customerPhone'] ?? '');
        if (!phone)
            continue;
        ...
        try {
            ...
            console.error(`[sendPickupReminders] Preorder ${doc.id} SMS failed:`, err.message);
        }
    }
});
// ── sendWeeklyDigest ──────────────────────────────────────────────────────────
...
exports.sendWeeklyDigest = (0, scheduler_1.onSchedule)({ schedule: '0 14 * * 1', secrets: [secrets_1.sendgridApiKey] }, async () => {
    const db = (0, firestore_1.getFirestore)();
    const siteUrlStr = secrets_1.siteUrl.value() || 'https://thepawnshop.ca';
    ...
    const itemSnap = await db.collection('items')
        ...
        .limit(20)
        .get();
    const digestItems = itemSnap.docs
        .filter((d) => {
        const data = d.data();
    ...
    for (const user of emailRecipients) {
        try {
            const sent = await (0, email_1.dispatchEmail)(String(user.data()['email']), subject, html);
            if (sent)
                successCount++;
        }
        catch (err) {
            console.error(`[sendWeeklyDigest] Email failed for user ${user.id}:`, err.message);
        }
    }
    await db.collection('auditLogs').add({
